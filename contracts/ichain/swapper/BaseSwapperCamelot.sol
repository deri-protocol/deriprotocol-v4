...
contract BaseSwapperCamelot is Admin {
    ...
    // Path is constructed as: [tokens[0], tokens[1], ... tokens[N-1]]
    function setPath(address[] memory tokens) external _onlyAdmin_ {
        ...
        for (uint256 i = 1; i < length; i++) {
            require(factory.poolByPair(tokens[i-1], tokens[i]) != address(0), 'Invalid path');
            path = abi.encodePacked(path, tokens[i]);
        }
        paths[tokens[0]][tokens[length-1]] = path;

        // Backward path
        path = abi.encodePacked(tokens[length-1]);
        for (uint256 i = length - 1; i > 0; i--) {
            path = abi.encodePacked(path, tokens[i-1]);
        }
        paths[tokens[length-1]][tokens[0]] = path;

        IERC20(tokens[0]).approve(address(router), type(uint256).max);
        IERC20(tokens[length-1]).approve(address(router), type(uint256).max);
    }
