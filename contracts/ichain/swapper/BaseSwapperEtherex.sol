...
contract BaseSwapperEtherex is Admin {

    using SafeERC20 for IERC20;

    IEtherexFactory public immutable factory;

    IEtherexSwapRouter public immutable router;
    ...
    constructor (address factory_, address router_, address tokenWETH_) {
        factory = IEtherexFactory(factory_);
        router = IEtherexSwapRouter(router_);
    ...
    external payable returns (uint256 result1, uint256 result2)
    {
        ...
        IEtherexSwapRouter.ExactInputParams memory params = IEtherexSwapRouter.ExactInputParams({
    ...
    external payable returns (uint256 result1, uint256 result2)
    {
        ...
        IEtherexSwapRouter.ExactOutputParams memory params = IEtherexSwapRouter.ExactOutputParams({
